...
export class RbacRepository implements IRbacRepository {
  ...
  async getAllUserRoles(): Promise<{ userId: string; roleName: string }[]> {
    const query = `
      SELECT user_id as "userId", role_name as "roleName"
      FROM user_roles
      ORDER BY user_id ASC
    `;
    const result = await this.db.query<{ userId: string; roleName: string }>(query);
    return result.rows;
  }
}
